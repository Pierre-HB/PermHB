...
namespace salseman256 {
    ...
    threadStream createThreadStream() {//type permetant un monitoring en temps reel
        ...
        stream._bestDistance = _mm256_set1_ps(factoriel::nbCityMax * 2);
        stream._bestPermNb = _mm256_setzero_si256();
        stream._worstDistance = _mm256_setzero_ps();
        stream._worstPermNb = _mm256_setzero_si256();
        stream._currentPermNb = _mm256_setzero_si256();
        stream.running = true;
        return stream;
    }

    void computePermutationSalseman(int n, int NCPUT, int commonePerm[], __m256 _constantDistance, __m256 _localDistanceMatrix[], threadStream& stream) {
        //calcul une partie de (NCPUT!)*nbUnderThreads permutations
        if (n == 1) {
            __m256 _distance = _constantDistance;
            _distance = _mm256_add_ps(_distance, _localDistanceMatrix[commonePerm[0] * factoriel::nbCityMax + commonePerm[1]]);//distance += localDistanceMatrix[commonePerm[i] * nbCityMax + commonePerm[i + 1]];
            _distance = _mm256_add_ps(_distance, _localDistanceMatrix[commonePerm[NCPUT] * factoriel::nbCityMax + commonePerm[0]]);//distance += localDistanceMatrix[commonePerm[NCPUT] * nbCityMax + commonePerm[0]];

            __m256 mask;
            mask = _mm256_cmp_ps(_distance, stream._bestDistance, _CMP_LT_OQ);//_distance < *_bestDistance
            stream._bestDistance = _mm256_blendv_ps(stream._bestDistance, _distance, mask);// = _distance < *_bestDistance ? _distance : *_bestDistance
            stream._bestPermNb = _mm256_blendv_epi8(stream._bestPermNb, stream._currentPermNb, _mm256_castps_si256(mask));// = _distance < *_bestDistance ? *_permNb : *_bestPermNb

            mask = _mm256_cmp_ps(_distance, stream._worstDistance, _CMP_GT_OQ);//_distance > *_worstDistance
            stream._worstDistance = _mm256_blendv_ps(stream._worstDistance, _distance, mask);// = _distance > *_worstDistance ? _distance : *_worstDistance
            stream._worstPermNb = _mm256_blendv_epi8(stream._worstPermNb, stream._currentPermNb, _mm256_castps_si256(mask));// = _distance > *_worstDistance ? *_permNb : *_worstPermNb
       

        
        }
        else {
            computePermutationSalseman(n - 1, NCPUT, commonePerm, _mm256_add_ps(_constantDistance, _localDistanceMatrix[commonePerm[n-1] * factoriel::nbCityMax + commonePerm[n]]), _localDistanceMatrix, stream);
            for (int i = 0; i < n - 1; i++) {
                if ((n % 2) == 0) {
                ...
                stream._currentPermNb = _mm256_add_epi32(stream._currentPermNb, _mm256_set1_epi32(1));//permNb = permNb + 1;
                computePermutationSalseman(n - 1, NCPUT, commonePerm, _mm256_add_ps(_constantDistance, _localDistanceMatrix[commonePerm[n - 1] * factoriel::nbCityMax + commonePerm[n]]), _localDistanceMatrix, stream);
            }
        }
    }
    ...
    void computePartialSalseman(unsigned long long n, float distanceMatrix[], int nbCity, int NCPUT, threadStream& stream) {
        ...
        __m256 _partialDistance = _mm256_setzero_ps();//puisque certaine ville seront toujours placer au même endroit dans les diferents chemin, ont peut calculer la distance parcourue une seul fois
        for (int i = NCPUT; i < nbCity - 1; i++) {
            _partialDistance = _mm256_add_ps(_partialDistance,
                _mm256_set_ps(
        ...
        __m256 _localDistanceMatrix[factoriel::nbCityMax][factoriel::nbCityMax];
        for (int i = 0; i < NCPUT; i++) {
            for (int j = 0; j < NCPUT + 1; j++) {//ATTENTION la matrice N'EST PAS symetrique
                _localDistanceMatrix[i][j] = _mm256_set_ps(
        ...
        for (int j = 0; j < NCPUT + 1; j++) {
            _localDistanceMatrix[NCPUT][j] = _mm256_set_ps(
    ...
    void computePartialSalsemanThread(int offset, float distanceMatrix[], int NCPUT, int nbCity, int nbThreads, threadStream* stream) {
        ...
        (*stream).running = true;
        (*stream)._bestDistance = _mm256_set1_ps(factoriel::nbCityMax * 2);//plus grand que parcourire les diagonal autant de fois qu'il n'y a de ville
        (*stream)._bestPermNb = _mm256_setzero_si256();
        (*stream)._worstDistance = _mm256_setzero_ps();
        (*stream)._worstPermNb = _mm256_setzero_si256();
        (*stream)._currentPermNb = _mm256_setzero_si256();
